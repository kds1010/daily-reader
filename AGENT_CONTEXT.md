...
- `KeyHintsMac`はDaymeldとは独立した軽量メニューバーmacOSユーティリティで、Accessibility APIが公開する前面アプリの可視操作要素へキーボードラベルを表示し、`AXPress`または限定的な座標クリックを実行する。macOS 14+、Accessibility許可が必要で、OCR・Screen Recording・Input Monitoringは使用しない。canvas等の非公開UIは対象外とする。`scripts/build_keyhints_release.py`でarm64成果物を生成する。
...
- Daymeldカードを開くと「現在の進捗」と「やりとり」を分け、ユーザー・Agent・システム進捗を話者ごとに表示する。待機中・実行中・判断待ちの直近3件を表示し、カードから全履歴も展開できる。待機中・実行中・判断待ち、および作業環境を保持した失敗状態へ追加指示を送れる。tanomiカードは展開時だけ依頼内容と結果またはエラーを全文表示し、セッションが残る完了・失敗・停止済みタスクへ追加指示を送れる。
