...
- Agentワーカー起動時は、前プロセスの再起動で`running`のまま残ったタスクを復旧可能な失敗状態へ移し、永久に実行中表示へ残さない。
...
- Agentタスクはカード全体の左右スワイプまたはボタンでアーカイブでき、7日間アーカイブ一覧から確認できる。期限後は関連履歴ごと自動削除し、アーカイブ後に進捗や履歴が更新されると通常一覧へ自動で戻す。
