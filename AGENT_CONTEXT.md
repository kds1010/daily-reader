...
- 2026-09-08のSideStore更新失敗は、ユーザー提示の文言から`OperationError 1006`（端末UDID取得不可）と特定した。0.6.3の`fetchUDID()`失敗や自己再署名時のペアリング情報欠落で発生する。ペアリング失効の契機やVPN経路の不調までは未確定。配信検証の成功だけでは解消しない。接続可能なiPhoneでSideStoreの`Reset Pairing File`、iLoaderの`Delete Stored Pairing → Refresh`と再ペアリング、`Manage Pairing File`の既存SideStore行への`Place`、端末でのRefresh・Daymeld UPDATEの順に確認する。HealthKit自己ビルド版は維持し、公式SideStoreの再インストールやアプリ削除を行わない。端末未接続時は既存ペアリングを削除せず、実機修復を未実施と報告する。公式資料・VPN確認・検証境界は`ios/README.md`の1006診断を参照する。
