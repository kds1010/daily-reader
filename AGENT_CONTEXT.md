...
- Codexの各呼び出しは`ai_usage.py`で工程・内部ID・指定モデル・実行時に報告された利用量・所要時間を、継立と共通のローカルJSONL台帳へ記録する。ニュース、会話の抽出・要約・補正・検証、生活調査、Agentの計画・実装・競合・デプロイを区別する。本文や秘密は保存せず、未知の実モデル・費用・トークン数を指定モデルや0で補完しない。台帳の場所、対象、強制終了時の限界は[AI利用量の計測](docs/ai-usage.md)を参照する。
...
- 記事候補・不要フィードバック・選定方針が同じなら、保存済み生成時刻から24時間は再生成しない。自身の選定履歴・取得順序は失効条件にしない。24時間後は次の更新機会に鮮度確認と選定ローテーションを行い、新着・本文変更・新しい不要指定は待たずに反映する。サーバー再起動でも保存済み結果を再利用し、同じ出力先の並行生成はファイルロックで防ぐ。候補が空の場合はCodexを呼ばない。
...
- 会話タブは録音単位の一覧を中心とし、確認済み録音日時、要点、カテゴリ別の抽出件数・代表項目、録音開始のGPS照合状態を表示する。取り込みフォームは別の導線から開く。既存の`topics.summary`は発話の単純連結であり、生成した会話要約として使わない。
...
- 要約・候補抽出の出力スキーマにも、各呼び出しで渡した短い発話IDだけを`enum`で指定する。元のスキーマは変更せず一時ファイルへ制約を追加し、受信後の未知ID・重複・件数の検証も維持する。成功済みの要約や候補をこの制約追加だけで再生成しない。
...
2026-09-09のユーザー指示により、文字起こし補正に限り、本人と確認された過去の関連発言の
短い断片を、現在の録音とは別の参考情報としてCodexへ渡せるようにする。冒頭の
「ほかの録音は渡さない」という従来の送信範囲には、この限定された例外を適用する。
通常の要約・候補抽出へ他録音全体は渡さず、原音・GPS・健康・カレンダーの送信範囲も増やさない。

### 文字起こしの文脈補正

- 文字起こし後の補正案生成と、独立したテキスト意味照合を追加する。元の発話本文・ID・話者・音声内時刻を上書きせず、採用・保留と根拠を補正版へ保存する。音声との照合を実施したとは説明しない。
- 採用できる補正だけを要約・候補抽出の本文へ適用し、引用は原文、補正文とその版は別フィールドに持つ。補正だけで確認済み候補、既存の採否、暮らしの項目、GPSの判断根拠を変更しない。
- 過去情報は、本人に対応付けられ、録音日時が確認済みで対象より前の関連発言に限定する。同名・同じ話者番号から本人を推測しない。過去情報を今回の発言・予定・約束へ変えない。本人対応がない録音では同じ録音内の前後関係だけを使用する。
- 補正の状態・原文・補正案・採用本文・検証理由・参照元は会話詳細から確認する。音声認識の漏れは文章補正で復元できず、従来の認識診断も維持する。仕様と検証の限界は[文脈補正](docs/conversation-correction.md)を参照する。
...
- 2026-09-10のユーザー指示により、会話画面から正表記・読み・確認済み誤表記を登録する用語辞書と、発言単位の本人訂正を追加した。辞書登録は明示操作だけとし、AI補正結果や出現回数から自動学習しない。用語100件、正表記/読み各80文字、誤表記5件まで。辞書全体版・用語履歴、本人訂正/取り消しの版と原文を会話DBへ保持する。
- ローカルfaster-whisperのhotwordsを各窓へ渡し、実token上限内で選択した語と辞書版を診断情報に記録する。前文引継ぎ無効の設定は維持する。二次三次補正へは関連する登録語を過去の本人発言と別に渡し、数字・否定・意味変更の保護を維持する。登録・訂正だけではCodex呼び出しや既存録音の一括再解析をしない。
