...
- tanomiチケットも既定で折り畳み、タイトル・状態・リポジトリだけを一覧表示する。展開時だけ依頼内容と結果またはエラーを全文表示し、5秒ごとの更新後も同じチケットの展開状態を維持する。
