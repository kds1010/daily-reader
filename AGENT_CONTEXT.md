...
- 通常のAgentタスクは既定モデルで読み取り専用の実装計画を作成し、同じスレッドを`gpt-5.6-luna`（low）で再開して実装・検証・コミットする。要件深掘りと完了後の質問は既定モデルを維持する。
...
- Agentタブ上部には、Codex app-serverのアカウントAPIから取得した利用枠ごとの使用率、残量、リセット時刻を表示する。
- ヘッダーには、稼働中のパッケージ版とGitコミットを組み合わせたデプロイバージョン、サーバー起動日時、および初期表示・手動再読み込み時の画面更新日時を表示する。
- 各タブは、画面上端から下へ引っ張る操作でもヘッダーの再読み込みボタンと同じ内容を更新する。
- Agentタスクはカードを既定で折り畳み、状態アイコン、現在フェーズ、更新時刻を一覧表示する。カードを開くと待機中・実行中・判断待ちの直近3件のイベントを表示し、5秒ごとに自動更新する。カードから全履歴も展開でき、「やりとりを非表示」または表示中のやりとりのタップで閉じられる。待機中・実行中・判断待ち、および作業環境を保持した失敗状態へ追加指示を送れる。実行中の指示は次のCodexターンで同じスレッドへ渡す。
...
- Agentタスクの投入画面は、省スペースなタスク入力、リポジトリ選択、依頼ボタンだけを表示し、即時実行する。
