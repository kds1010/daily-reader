...
- macOS版はMac mini API上のAgent、Planner、Gmail、ニュース、tanomi、Codex利用状況と、iPhoneから同期済みの健康集計を共有する。MacにはHealthKitデータストアがないため、HealthKit同期とトークン入力はiPhone版だけに表示する。画面内容は`Command`+`=`（`Command`+`+`も可）／`Command`+`-`で80%から140%まで10%刻みで拡大・縮小し、`Command`+`0`で100%へ戻せる。倍率は次回起動時も維持する。macOS版はApp Sandboxと外向きネットワークだけを許可し、HealthKit entitlementを含めない。
