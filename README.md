...
「会話」では、録音ごとに日時・話した内容の要点・抽出項目・GPSの照合状況を確認できます。
詳細では要約の根拠発言と、確認待ち・保存済み・追加済みの項目を同じ会話から読めます。
何がタスクや予定・関心になるかの基準と、要約更新の手順は
[会話ごとの振り返り](docs/conversation-overview.md)を参照してください。
...
新規録音は文字起こし後に文脈補正と意味の点検を行い、その後Codex整理を自動開始し、
...
録音日時、話者、発話時刻、文字起こしです。文脈補正では、本人と確認された過去の関連発言を
短く選んで参考にできます。MP3原音、GPS、ファイル名や他録音全体は渡しません。
Codexは一時セッション、読み取り専用、ユーザー設定・ルール無効で実行し、
...
- Codex CLIによる「今日のハイライト」と、公式リリースの製品別日本語まとめ（候補・不要指定の変更時、または前回生成から24時間経過後に生成）
...
Codex CLIがログイン済みの場合は、更新時に注目記事から今日のハイライトを生成します。Snowflake、Databricks、dbt、Apache Icebergなどの公式リリースは製品別に束ね、英語の記事も日本語で要約して原文リンクを添えます。同じ候補・不要フィードバックなら、サーバー再起動後も前回生成から24時間は再利用します。自身の選定履歴だけでは再生成せず、24時間後の更新時に鮮度と選定を見直します。新着や本文の変更、新しい不要指定は待たずに反映します。実行にはCodexの利用量を消費しますが、`gpt-5.6-luna`を低推論設定で1回だけ呼び出し、ハイライトと公式リリースまとめを同時に生成します。Codexは`--ephemeral`、`--sandbox read-only`、構造化出力で呼び出され、記事本文中の命令を無視するよう指示されます。
