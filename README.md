...
録音の詳細で「LLMでタスク・決定・アイデアを整理」を明示的に選ぶと、文字起こしから
タスク、フォローアップ、決定事項、アイデア、困りごとを抽出し、「音声インボックス」へ
根拠発言付きで表示します。利用する場合はOpenAI APIキーを
`secrets/openai-api-key.txt`へ保存してください。既定モデルは`gpt-5-mini`で、サーバー起動時の
`--conversation-insight-model`で変更できます。OpenAIへ送るのは録音日時、話者、発話時刻、
文字起こしだけです。MP3原音、GPS、ファイル名、ほかの録音は送信しません。APIリクエストは
保存無効かつツールなしで実行します。送信前にはアプリ内で確認し、候補のタイトル・詳細・
