...
録音の詳細で「Codexでタスク・決定・アイデアを整理」を明示的に選ぶと、文字起こしから
タスク、フォローアップ、決定事項、アイデア、困りごとを抽出し、「音声インボックス」へ
...
この処理の子プロセスから除外します。送信前にはアプリ内で確認し、候補のタイトル・詳細・
担当者・期限を編集してから保存、破棄、通常タスクへの追加、Agentへの依頼を選びます。
候補が自動でタスク化・実行されることはありません。
...
- ファイル更新日時や取り込み日時は録音日時として使用しない。既存の録音日時も未確認として扱い、Codexへはnullを渡す。日時不明時は相対期限を日付へ変換せず、原文を保持する。APIで録音日時を指定する場合は、タイムゾーン付き`recorded_at`と明示確認の`recorded_at_confirmed=true`が必要。
