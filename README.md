...
Daymeldとtanomiのタスクカードは更新時刻順の同じ一覧へ表示され、Daymeldはミント、tanomiは紫の出所バッジと左端アクセントで区別できます。どちらも既定では折り畳まれ、状態アイコン、リポジトリ、更新時刻を一覧で確認できます。Daymeldカードを開くと「現在の進捗」と「やりとり」を分けて表示し、tanomiカードは開いたときだけ依頼内容と結果（またはエラー）を全文表示します。
