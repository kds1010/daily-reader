...
struct DaymeldFixture {
    ...
    static func conversationExamples() -> [ConversationRecording] {
        ...
        [{"id":"fixture-conversation-1","filename":"匿名の会話.ogg","byte_size":120000,"status":"completed","created_at":"2026-09-02T00:00:00Z","recorded_at":"2026-09-01T09:30:00+09:00","recorded_at_verified":true,"recorded_at_source":"soundcore_drive_folder_name","duration_seconds":780,"source_type":"audio","insight_status":"completed",
          ...
          "utterances":[{"id":"fixture-u1","speaker":"話者1","start_seconds":12,"end_seconds":16,"text":"明日までに資料を送ります。","context":"説明用","topic":"準備"},{"id":"fixture-u2","speaker":"話者2","start_seconds":17,"end_seconds":22,"text":"比較表を使って検討しましょう。","context":"説明用","topic":"準備"},{"id":"fixture-u3","speaker":"話者1","start_seconds":23,"end_seconds":27,"text":"星身計画の資料です。","context":"説明用","topic":"用語","user_correction_revision":2,"user_correction":{"id":"fixture-user-correction","revision":2,"original_text":"星身計画の資料です。","corrected_text":"星見計画の資料です。"}}],
          "insight_items":[{"id":"fixture-i1","recording_id":"fixture-conversation-1","kind":"task","title":"資料を送る","detail":"説明用の確認待ち項目です。","certainty":"explicit","status":"awaiting_review","source":"codex","evidence":[{"position":0,"utterance_id":"fixture-u1","quote":"明日までに資料を送ります。","corrected_quote":"明日までに、資料を送ります。","correction_revision_id":"fixture-older-correction","correction_is_snapshot":true,"speaker":"話者1","start_seconds":12,"end_seconds":16}]},{"id":"fixture-i2","recording_id":"fixture-conversation-1","kind":"decision","title":"比較表で検討する","detail":"説明用の保存済み項目です。","certainty":"explicit","status":"kept","source":"codex","evidence":[{"position":0,"utterance_id":"fixture-u2","quote":"比較表を使って検討しましょう。","speaker":"話者2","start_seconds":17,"end_seconds":22}]},{"id":"fixture-i3","recording_id":"fixture-conversation-1","kind":"research","title":"比較軸を調べる","detail":"説明用の追加済み項目です。","certainty":"explicit","status":"approved","source":"codex","approved_target":"life","approved_item_id":"fixture-life","evidence":[{"position":0,"utterance_id":"fixture-u2","quote":"比較表を使って検討しましょう。","speaker":"話者2","start_seconds":17,"end_seconds":22}]}]},
        ...
        """#
        var recordings = (try? JSONDecoder().decode([ConversationRecording].self, from: Data(wire.utf8))) ?? []
        guard recordings.count == 4 else { return recordings }
        let correction = #"""
        {"status":"completed","revision_id":"fixture-correction-1","corrected_count":1,"retained_count":1,"flagged_count":1,"context_count":1,"error":null,"completed_at":"2026-09-01T10:05:00+09:00","automatic_blocked":false,
         "items":[
          {"utterance_id":"fixture-u1","original_text":"明日までに資料を送ります。","proposed_text":"明日までに、資料を送ります。","corrected_text":"明日までに、資料を送ります。","status":"accepted","reason":"説明用：文の意味を変えず、区切りの表記を整えました。","verification":"verified","context_ids":[]},
          {"utterance_id":"fixture-u2","original_text":"比較表を使って検討しましょう。","proposed_text":"比較票を使って検討しましょう。","corrected_text":null,"status":"retained","reason":"説明用：用語の表記を変更する確証がありません。","verification":"uncertain","context_ids":["fixture-context-1"]}],
         "contexts":[{"id":"fixture-context-1","source_type":"confirmed_self_utterance","source_id":"fixture-prior-u1","recording_id":"fixture-conversation-prior","recorded_at":"2026-08-31T09:30:00+09:00","title":"説明用の参照発言（本人確認済み）"}]}
        """#
        recordings[0].transcriptionMetadata = ConversationTranscriptionMetadata(model: "説明用モデル", warnings: [], vocabularyRevision: 1, vocabularyTermIDs: ["example-term"], vocabularyOmittedCount: 2)
        recordings[0].correction = try? JSONDecoder().decode(ConversationCorrection.self, from: Data(correction.utf8))
        let failed = #"""
        {"status":"failed","revision_id":null,"corrected_count":0,"retained_count":0,"flagged_count":0,"context_count":0,"error":"説明用：補正処理に失敗しました。","completed_at":null,"automatic_blocked":true,"items":[],"contexts":[],"context_message":"本人として確認された関連する過去の文脈がありません"}
        """#
        recordings[2].correction = try? JSONDecoder().decode(ConversationCorrection.self, from: Data(failed.utf8))
        let prior = #"""
        {"id":"fixture-conversation-prior","filename":"説明用の過去の会話.txt","byte_size":80,"status":"completed","source_type":"transcript","created_at":"2026-08-31T01:00:00Z","recorded_at":"2026-08-31T09:30:00+09:00","recorded_at_verified":true,"utterances":[{"id":"fixture-prior-u1","text":"説明用です。表の名称は比較表と呼んでいます。","speaker":"本人確認済みの説明用話者","start_seconds":0,"end_seconds":1,"context":"説明用","topic":"用語"}]}
        """#
        if let previous = try? JSONDecoder().decode(ConversationRecording.self, from: Data(prior.utf8)) {
            recordings.append(previous)
        }
        return recordings
    }
